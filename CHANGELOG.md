...
- **Added**: `packages/api/src/serialization.ts` — `serializeEntityRecord` (camelCase core record → snake_case response) and `deserializeEntityInput` (snake_case request body → camelCase core input), with semantic renames for deal (`name`↔`title`), stage (`position`↔`stageOrder`), and note (`body`↔`content`, `user_id`↔`createdByUserId`).
...
- **Changed**: Hardened Plan B release readiness follow-ups with release E2E validation, public-repository provenance checks, package artifact verification for entrypoints/exports/bin/files allowlists, clearer release dry-run diagnostics, and Codex execution/coverage docs.
