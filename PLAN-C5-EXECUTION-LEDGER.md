...
| `packages/core/src/schema-engine/engine.ts` | 4, 6 | Task 4 explicit schema engine dependencies, migration authority contract, structured missing-authority errors, contextual authority boundary, and fail-closed placeholders. Task 6 preview classification from custom-field metadata, adapter snapshot, and paginated ledger state; conservative destructive classification and checksum-bound apply preflight. | Task 4 focused core tests/typecheck/build. Task 6 focused core tests/typecheck/build and SDK parity/direct tests. | Task 4 schema/spec PASS, tenant/security PASS, code review APPROVED. Task 6 final schema/spec PASS and code review APPROVED. |
| `packages/core/src/schema-engine/engine.test.ts` | 4, 6 | Task 4 authority boundary, missing authority, destructive confirmation, stale checksum, rollback placeholder, and fail-closed field operation coverage. Task 6 preview classification, no-side-effect preview, ledger pagination, conservative merge, checksum preflight, and adapter support regressions. | Task 4 focused core tests. Task 6 focused core tests. | Final Task 4 reviews PASS/APPROVED. Task 6 final reviews PASS/APPROVED. |
| `packages/core/src/services/index.ts` | 4, 6 | Task 4 wires explicit migration authority into `OrbitSchemaEngine` and adds runtime-adapter service constructor. Task 6 wires schema adapter source for preview classification. | Task 4 focused core tests/typecheck/build. Task 6 core typecheck/build. | Task 4 schema/spec PASS; tenant/security PASS. Task 6 final schema/spec PASS. |
...
| `packages/sdk/src/__tests__/direct-transport.test.ts` | 4, 6 | Task 4 DirectTransport migration authority unavailable behavior and no adapter authority use. Task 6 fixture checksum update after core apply checksum binding. | Task 4 SDK direct tests. Task 6 SDK direct/parity tests. | Tenant/security PASS. Task 6 code review APPROVED. |
