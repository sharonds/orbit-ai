...
export function assertSafePostgresE2eUrl(databaseUrl: string): void {
  ...
  if (url.protocol !== 'postgres:' && url.protocol !== 'postgresql:') {
    throw new Error(`Refusing to run Postgres e2e tests with unsupported ${url.protocol} URL.`)
  }

  if (url.searchParams.size > 0) {
    throw new Error('Refusing to run Postgres e2e tests with connection-string query parameters.')
  }
