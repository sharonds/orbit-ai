...
Current baseline commit: `78d6108` (main) / `working tree` (local MCP implementation in progress)
...
- `@orbit-ai/mcp` is now implemented locally: 23 core tools, stdio + HTTP transport seams, `_untrusted` resources, structured MCP errors, gated stubs for unavailable capabilities, and 93 passing tests
...
1. All five core packages are implemented:
   ...
   - `@orbit-ai/mcp` — implemented locally and under review/final integration
...
3. Next:
   ...
   - integrate the CLI `mcp serve` wrapper with the implemented package
   - publish all 5 packages together as `0.1.0-alpha.0` to npm
