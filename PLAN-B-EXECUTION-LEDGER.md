...
|---|---|---|---|---|
| 1. Release-workflow regression tests first | `scripts/release-workflow.test.mjs` | `node --test scripts/release-workflow.test.mjs` | Pass | 24 tests pass; release E2E gate, provenance precondition, Changesets ignore, and CI E2E regex tests are included. |
| 2. Package verifier regression tests first | `scripts/release-workflow.test.mjs` | `node --test scripts/release-workflow.test.mjs` | Pass | Fixture tests cover string-form and object-form `bin`, invalid object-form `bin`, missing `license`, missing `README.md`, missing `LICENSE`, missing/empty `files`, allowlist omissions, private/non-orbit skips, multiple failures, and malformed manifest paths. |
| 3. Dry-run diagnostics regression tests first | `scripts/release-workflow.test.mjs` | `node --test scripts/release-workflow.test.mjs` | Pass | Fixture tests cover spawn failure, signal termination, non-zero status, and malformed package JSON path diagnostics. |
| 4. Implement release workflow, Changesets, and CI gates | `.github/workflows/release.yml`, `.github/workflows/ci.yml`, `.changeset/config.json` | `node --test scripts/release-workflow.test.mjs`; `pnpm changeset status` | Pass | Release validation runs `@orbit-ai/e2e` before package tests; publish checks public repository before provenance publish; CI regexes include release-sensitive files and Postgres journey helpers; Changesets ignores `@orbit-ai/e2e`. |
...
| 6. Implement artifact verifier | `scripts/verify-package-artifacts.mjs` | `node --test scripts/release-workflow.test.mjs`; `node scripts/verify-package-artifacts.mjs`; `pnpm release:verify-artifacts` | Pass | Verifier checks metadata, README/LICENSE, string/object bins, exports, entrypoints, files allowlist coverage, private/non-orbit skips, and path-aware JSON parse failures. |
...
| 12. Plan/execution concordance review | `PLAN-B-EXECUTION-LEDGER.md` | `git diff --name-only`; `node --test scripts/release-workflow.test.mjs`; stale-pattern search | Pending final subagent review | Ledger created; final concordance subagents review after this file exists. |
