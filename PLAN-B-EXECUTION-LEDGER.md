...
| 6. Implement artifact verifier | `scripts/verify-package-artifacts.mjs` | `node --test scripts/release-workflow.test.mjs`; `node scripts/verify-package-artifacts.mjs`; `pnpm release:verify-artifacts` | Pass | Verifier checks metadata, README/LICENSE, string/object bins, invalid declared bins, exports, entrypoints, ordered literal/glob/negated files allowlist coverage, private/non-orbit skips, and path-aware JSON parse failures. |
